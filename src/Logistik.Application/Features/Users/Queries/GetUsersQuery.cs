...
public record GetUsersQuery(int Page = 1, int PageSize = 20, string? Search = null) : IRequest<PaginatedResult<UserDto>>;
...
public class GetUsersQueryHandler : IRequestHandler<GetUsersQuery, PaginatedResult<UserDto>>
{
    ...
    {
        var (items, total) = await _uow.Users.GetPagedAsync(request.Page, request.PageSize, request.Search, ct);
